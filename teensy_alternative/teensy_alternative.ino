...
void loop() {
  ...
  if (millis() - timestamp > debounceTime) {
    ...
    int rightKnee = analogRead(A3);
    int leftKnee = analogRead(A4);
    Serial.print(rightKnee);
    Serial.print(",");
    Serial.print(leftKnee);

    if (rightKnee < rightTH && rightISOPEN == true) {
      leftISOPEN = false;
      Keyboard.press(KEY_RIGHT);
      Keyboard.release(KEY_RIGHT);
      timestamp = millis();
    }
    else rightISOPEN = true;

    if (leftKnee < leftTH && leftISOPEN == true) {
      rightISOPEN = false;
      Keyboard.press(KEY_LEFT);
      Keyboard.release(KEY_LEFT);
      timestamp = millis();
    }
    else leftISOPEN = true;
